...
impl<T: ?Sized> RawEntity<T> {
    /// Construct a borrow of this [RawEntity], returning the raw borrow components.
    ///
    /// # Safety
    ///
    /// Callers may only use the returned pointer while holding the corresponding [BorrowRef],
    /// otherwise there is no protection against mutable aliasing of the underlying data.
    #[track_caller]
    #[allow(unused)]
    ...
    /// Construct a mutable borrow of this [RawEntity], returning the raw borrow components.
    ///
    /// # Safety
    ///
    /// Callers may only use the returned pointer while holding the corresponding [BorrowRefMut],
    /// otherwise there is no protection against mutable aliasing of the underlying data.
    #[track_caller]
