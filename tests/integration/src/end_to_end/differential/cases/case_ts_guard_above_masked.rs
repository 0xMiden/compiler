// Control for `case_ts_guard_above.rs`: byte-identical except that the table
// index is masked into range (`A31[g & 15]` instead of `A31[g]`), so the load,
// the mask, the five-bit extraction and the live cluster are all the same and
// the ONLY difference is that this file has no trapping edge. It compiles; its
// twin does not.
static A31: [u8; 31] = [
