...
// not see a cycle. Natively this is ordinary recursion. The table is read
...
// which would turn the dispatch back into a call-graph cycle.
...
fn rec_a(n: u32, s: u64) -> u64 {
    ...
    let f = black_box(&STEPS)[((s >> 3) % 3) as usize];
...
fn rec_b(n: u32, s: u64) -> u64 {
    ...
    let f = black_box(&STEPS)[(s % 3) as usize];
...
pub extern "C" fn entrypoint(input1: u32, input2: u32) -> u32 {
    ...
    let f = black_box(&STEPS)[(input2 % 3) as usize];
