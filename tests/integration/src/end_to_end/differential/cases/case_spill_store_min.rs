// MINIMAL `-Oz` REPRODUCER of F17 (campaign 28, W3), reduced from
// `case_prog_threefish.rs`: four u64 words mixed by EIGHT rotation rows
// (sixteen distinct rotation constants) inside a kept round-group loop, with
// subkey and tweak injection from two small arrays.  At
// `--optimize=size-min` the spill analysis requests spills, splits no edge,
// and the emitter then fails on the spill store itself.  See the ignored test
// in tests/opt_levels.rs.
#[unsafe(no_mangle)]
