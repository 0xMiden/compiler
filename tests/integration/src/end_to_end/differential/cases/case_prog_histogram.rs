// Two-pass entropy coder front-end (campaign 21, cliff shape: sequential
// loops sharing constants).  Pass one builds a sixteen-bucket histogram of a
// 96-byte buffer and six u64 statistics; pass two turns the histogram into
// cumulative offsets and emits a packed code stream, reusing the SAME shift
// constants the first pass used for bucket selection — so every constant is
// live from before the first loop, across it, and into the second loop, and
// the six statistics are combined in ONE expression inside the emission loop
// and folded into the result after it.
const B0: u32 = 3;
const B1: u32 = 7;
const B2: u32 = 11;
const B3: u32 = 17;
