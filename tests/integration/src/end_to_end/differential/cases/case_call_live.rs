// Twenty felts (ten u64s) live across two calls to a pinned
// `#[inline(never)]` helper taking three u64 arguments that are all used
// again after the calls (Copy-constrained exec operands). The `fetch_add(0)`
// side effect keeps the calls from sinking to their uses. Guards the
// arity-3-with-copies scheduling path (LinearStackWindow / Linear / CopyAll)
// at call sites with more live state than the window holds.
...
pub extern "C" fn entrypoint(input1: u32, input2: u32) -> u32 {
    ...
    let t = mixer(v0, v1, v2);
    let u = mixer(v2, v0, v1);
    let r = (v1 ^ v9.rotate_left(1))
