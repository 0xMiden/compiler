// The same freight and the same guard as `case_ts_body_index.rs`, with the
// guard moved ONE STATEMENT up: it now sits immediately above the wide
// expression that consumes the whole eight-u64 cluster, instead of below it.
// The cluster is therefore live across the trapping edge at the point of its
// maximum pressure. `case_ts_guard_above_masked.rs` is the same file with the
// index masked into range — identical work, no trapping edge — and it is the
// control that says which of the two is responsible.
static A31: [u8; 31] = [
