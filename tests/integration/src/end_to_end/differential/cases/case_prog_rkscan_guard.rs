// Bounding sibling of `prog_rkscan` (campaign 21): the same Rabin-Karp
// scanner — three needles searched for in a 64-byte buffer with a rolling
// hash and four early exits — with THREE distinct rotate constants instead of
// five and FOUR fingerprint words instead of six.  This is the largest
// variant of the program that compiles at the default level (it still panics
// at `--optimize=basic`; see `prog_rkscan`).
const RA: u32 = 7;
const RB: u32 = 17;
const RC: u32 = 29;
const RD: u32 = RA;
