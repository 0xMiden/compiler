// A helper's RETURN VALUE feeds the guard: the call cannot trap, but the index
// it produces decides whether `entrypoint` does.
//
// The mirror image of `case_ts_helper_arg.rs` — there the trapping edge is
// inside the callee, here the callee is total and the trapping edge is in the
// caller, one use away from the call's result. The call boundary still forces
// a spill of everything live, so the cluster crosses both the call and the
// trapping edge it produces.
static A31: [u8; 31] = [
...
#[inline(never)]
fn mixer(x: u64, r: u32) -> u32 {
    (((x >> 57) ^ x.rotate_left(r)) & 31) as u32
}
...
pub extern "C" fn entrypoint(input1: u32, input2: u32) -> u32 {
    ...
    while t < 8 {
        ...
        let g = mixer(s, input1 & 31) as usize;
        s ^= A31[g] as u64;
    }
