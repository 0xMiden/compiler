// Trap edge in the BODY of a freight loop; guard kind: ARRAY INDEX.
//
// The freight is the campaign-20 recipe at the level `interact::cascade_spill`
// is built on: an eight-u64 cluster (v0..v7) defined before the loop, consumed
// inside it in ONE wide expression and used again after it, plus eight
// CSE-merged rotate count bands used before, inside and after. The guard is
// the last statement of the loop body, below the wide expression, and the
// cluster is live across it because every value is used again after the loop.
// Its index comes from five bits of the loop-carried accumulator, so it cannot
// be hoisted out of the loop. The index reaches 0..=31 against a 31-byte
// `static`, so exactly one of the thirty-two values traps.
static A31: [u8; 31] = [
