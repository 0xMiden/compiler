...
//
// `interact::cascade_spill` verbatim — two 8-trip loops, eight rotate count
// bands, an eight-u64 cluster — except that the first loop's `continue` arm is
// no longer empty: it asserts on four bits of the carried accumulator before
// continuing. The arm is the sole producer of the `WhileUnusedResult` ->
// `IndexSwitchRemoveUnusedResults` cascade, and a trapping edge is the least a
// statement can add to it (no value, no result column, just an exit), so this
// is the sharpest question the module asks of that cascade.
#[unsafe(no_mangle)]
