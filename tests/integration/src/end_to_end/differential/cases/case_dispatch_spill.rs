// Switch ARM MERGING (`SimplifySwitchFallbackOverlap`) on a dispatch that sits
// INSIDE a kept loop, with ten CSE-merged rotate count bands live across the
// switch. Five scattered arms of the sixteen share the default arm's body, so
// LLVM points those `br_table` entries at the fallback block and the pattern
// rebuilds the `cf.switch` without all five overlapping cases at once. The
// selector is `(input1 + i) & 15`, so a grid over `input1` starts the dispatch
// at a different arm on every entry and pins each arm's body against the native
// build. The bands are used before the loop, on the accumulator inside it (both
// in the arms and after the `match`), and after the loop.
#[unsafe(no_mangle)]
pub extern "C" fn entrypoint(input1: u32, input2: u32) -> u32 {
    ...
    while i < iters {
        ...
        match sel {
            ...
            15 => acc = acc.wrapping_add(acc.rotate_left(31) ^ 16),
