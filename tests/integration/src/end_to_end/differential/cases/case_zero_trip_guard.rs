// Passing guard at the zero-trip spill boundary. Ten masked rotate counts
// are shared between the pre-loop code and rotates of the loop-carried
// accumulator inside `while i < input2 % 97` — a ZERO-TRIP-CAPABLE top-test
// loop, so LLVM keeps its guard and a bypass edge around the loop (the
// corpus's `% 97 + 3` bounds become bottom-test loops without one) — with
// the 28/30 live-through pair and a second zero-trip-capable loop. Eleven
// counts hit the `zero_trip_frontier` panic; ten is the largest count that
// compiles, and it passes, including the pinned zero-trip and one-trip
// inputs of the `_repro` twin.
#[unsafe(no_mangle)]
