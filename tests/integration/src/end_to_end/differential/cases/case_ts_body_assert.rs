// Trap edge in the BODY of a freight loop; guard kind: `assert!`.
//
// Same freight and the same guard position as `case_ts_body_index.rs`, with
// the trapping edge written by hand instead of coming out of `core`: the
// assertion fails on one row in thirty-two of the loop-carried accumulator's
// top five bits. The value the non-trapping rows compute still folds that same
// five-bit slice back into the accumulator, so the guard's operand is live
// past the guard — a reload of it sunk below the assertion would be visible.
#[unsafe(no_mangle)]
