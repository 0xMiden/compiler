// Trap edge in the BODY of a freight loop; guard kind: `unreachable!()`.
//
// Same freight and the same guard position as `case_ts_body_index.rs`. This is
// the barest trapping edge the language offers: a runtime condition on the
// loop-carried accumulator's top five bits guarding an `unreachable!()`, which
// under `-Cpanic=immediate-abort` is exactly the wasm `unreachable` every other
// guard kind in this module decays to — so it is the control for "is it the
// EDGE or the guard's own work that moves a boundary".
#[unsafe(no_mangle)]
