...
// unreachable()` and a Rust panic edge, with the cluster crossing both.
//
// On the guest the two are indistinguishable — `-Cpanic=immediate-abort` makes
// every Rust panic a wasm `unreachable` too — so cfg-to-scf is handed two
// `ub.unreachable` terminators with no operands, which its `ReturnLikeOpKey`
// compares equal and `combine_exit` forwards into ONE exit block. On the host
// they are different mechanisms: `abort()` versus the header's panic handler.
// The freight is `case_ts_body_index.rs`'s; the explicit trap fires on one row
// in sixty-four and the panic edge on one in thirty-two, of two different
// slices of the loop-carried accumulator.
static A31: [u8; 31] = [
