// Trap edge in the BODY of a freight loop; guard kind: `get(..).unwrap()`.
//
// Same freight and the same guard position as `case_ts_body_index.rs` — an
// eight-u64 cluster consumed in one wide expression inside the loop and used
// again after it, eight rotate count bands crossing the loop — but the panic
// comes from `Option::unwrap` on a slice `get` rather than from an index
// expression's bounds check, so the trapping edge is produced by a different
// piece of `core`. Index 0..=31 against a 31-byte `static`.
static A31: [u8; 31] = [
