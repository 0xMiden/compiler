// Nine activations of a [MaybeUninit<u64>; 8192] frame (65536 bytes each,
// 589824 bytes in total) recursing through a function-pointer table (the
...
fn rec_a(n: u32, s: u64) -> u64 {
    let mut frame: [MaybeUninit<u64>; 8192] = [MaybeUninit::uninit(); 8192];
    let mid = (s % 8192) as usize;
    frame[0].write(s ^ 0x1111_1111_1111_1111);
    frame[8191].write(s.rotate_left(17));
    ...
    let b = unsafe { frame[8191].assume_init() };
...
fn rec_b(n: u32, s: u64) -> u64 {
    ...
    frame[8191].write(s.rotate_left(29));
