// Trap edge in the BODY of a freight loop; guard kind: DIVISION BY A BAND.
//
// Same freight and the same guard position as `case_ts_body_index.rs`, but the
// trapping edge is the zero-divisor test LLVM emits in front of a `u32 /`: the
// divisor is a five-bit mask of the loop-carried accumulator, which is zero on
// one row in thirty-two. That makes the trapping edge a sibling of the count
// bands themselves — a masked value used as a divisor rather than as a shift
// count.
#[unsafe(no_mangle)]
pub extern "C" fn entrypoint(input1: u32, input2: u32) -> u32 {
    ...
    while t < 8 {
        ...
        let b = ((s >> 57) & 31) as u32;
        s ^= (0x9e37_79b9u32 / b) as u64;
    }
