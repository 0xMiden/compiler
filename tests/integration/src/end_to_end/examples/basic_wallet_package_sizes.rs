...
fn basic_wallet_and_p2id() {
    ...
    expect!["14277"].assert_eq(stripped_mast_size_str(&account_package).as_str());
    ...
    expect!["14495"].assert_eq(stripped_mast_size_str(&tx_script_package).as_str());
    ...
    expect!["15345"].assert_eq(stripped_mast_size_str(&note_package).as_str());
