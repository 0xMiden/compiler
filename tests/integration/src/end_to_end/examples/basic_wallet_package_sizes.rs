...
fn basic_wallet_and_p2id() {
    ...
    expect!["13178"].assert_eq(stripped_mast_size_str(&account_package).as_str());
    ...
    expect!["14526"].assert_eq(stripped_mast_size_str(&tx_script_package).as_str());
    ...
    expect!["15353"].assert_eq(stripped_mast_size_str(&note_package).as_str());
    ...
    expect!["17785"].assert_eq(stripped_mast_size_str(&p2ide_package).as_str());
}
