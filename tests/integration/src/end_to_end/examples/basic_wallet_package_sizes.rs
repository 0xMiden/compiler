...
fn basic_wallet_and_p2id() {
    ...
    expect!["13958"].assert_eq(stripped_mast_size_str(&account_package).as_str());
    ...
    expect!["14457"].assert_eq(stripped_mast_size_str(&tx_script_package).as_str());
    ...
    expect!["15092"].assert_eq(stripped_mast_size_str(&note_package).as_str());
    ...
    expect!["17553"].assert_eq(stripped_mast_size_str(&p2ide_package).as_str());
}
