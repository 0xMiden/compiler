...
fn batch_kernel() {
    ...
    expect!["119947"].assert_eq(&stripped_mast_size_str(&package));
    ...
    //   creates two notes.
    {
        ...
        expect!["32801"].assert_eq(&cycles.to_string());
    ...
    // only tx1's authenticated input note remains in the commitment.
    {
        ...
        expect!["30751"].assert_eq(&cycles.to_string());
    ...
    // Scenario 3: a tampered BATCH_ID pre-image must be rejected by the Layer 1 hash check.
    {
        ...
        expect!["961"].assert_eq(&cycles.to_string());
    }
    ...
    // gate must reject the batch.
    {
        ...
        expect!["15614"].assert_eq(&cycles.to_string());
    }
}
