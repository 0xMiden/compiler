...
fn store_unaligned_u32() {
    ...
    fn run_test(
        ...
        let output = eval_package::<u32, _, _>(
            ...
            |trace| {
                ...
                eprintln!("word0: 0x{word0:0x}");
                eprintln!("word1: 0x{word1:0x}");

                assert_eq!(
                    word0, expected0,
                    "expected 1st overwritten word to be {expected0}, got {word0}, with index \
                     {idx}"
                );

                assert_eq!(
                    word1, expected1,
                    "expected 2nd overwritten word to be {expected1}, got {word1}, with index \
                     {idx}"
                );
    ...
    run_test(&package, &context, write_to, 1, 0xccbbaa11, 0x887766dd);
    ...
    run_test(&package, &context, write_to, 2, 0xbbaa2211, 0x8877ddcc);
    ...
    run_test(&package, &context, write_to, 3, 0xaa332211, 0x88ddccbb);
}
