    ...
    let test_fn = || {
        ...
        setup::build_entrypoint(link_output.component, &signature, |builder| {
            ...
            let ptr_u16_offset = builder
                .inttoptr(
                    addr_plus_2,
                    Type::from(PointerType::new(Type::U16)),
                    SourceSpan::default(),
                )
                .unwrap();
...
/// Tests the memory store intrinsic for stores of 8-bit (u8) values aligned to 2 bytes
#[test]
fn store_u8() {
    setup::enable_compiler_instrumentation();
    ...
    let test_fn = || {
        ...
        // Generate a `test` module with `main` function that stores two u8 values
        let signature = Signature::new(
            [AbiParam::new(Type::U8), AbiParam::new(Type::U8)],
            [AbiParam::new(Type::U32)], // Return u32 to satisfy test infrastructure
        );
        setup::build_entrypoint(link_output.component, &signature, |builder| {
            let block = builder.current_block();
            // Get the two input u8 values
            let value1 = block.borrow().arguments()[0] as ValueRef;
            let value2 = block.borrow().arguments()[1] as ValueRef;
            ...
            builder.store(ptr_u8, value1, SourceSpan::default()).unwrap();

            // Create pointer to offset 2 (aligned to 2 bytes)
            let addr_plus_2 = builder.u32(write_to + 2, SourceSpan::default());
            let ptr_u8_offset = builder
                .inttoptr(
                    addr_plus_2,
            ...
            // Store second u8 at offset 2
            builder.store(ptr_u8_offset, value2, SourceSpan::default()).unwrap();
        ...
        let output = eval_link_output::<u32, _, _>(
            ...
            |trace| {
                ...
                // Extract the u8 values from the u32
                let stored_u8_at_0 = (stored_u32 & 0xff) as u8;
                let stored_u8_at_2 = ((stored_u32 >> 16) & 0xff) as u8;
                ...
                println!(
                    "Stored u8 at offset 0: 0x{:02x} (expected 0x{:02x})",
                    stored_u8_at_0, value1
                );
                println!(
                    "Stored u8 at offset 2: 0x{:02x} (expected 0x{:02x})",
                    stored_u8_at_2, value2
                );

                // The u32 should contain both u8 values at their respective positions
                // In little-endian: byte 0 = value1, byte 2 = value2, bytes 1 and 3 = 0
                let expected_u32 = (value1 as u32) | ((value2 as u32) << 16);
...
fn store_u8_precise() {
    ...
    let value1: u8 = 0xaa; // Store at offset 0
    let value2: u8 = 0xbb; // Store at offset 2
    ...
    let test_fn = || {
        ...
        setup::build_entrypoint(link_output.component, &signature, |builder| {
            ...
            // After first store, check that only byte 0 was modified
            // Load the word containing bytes 0-3
            let ptr_u32 = builder
                .inttoptr(base_addr, Type::from(PointerType::new(Type::U32)), SourceSpan::default())
                .unwrap();
            let word_after_first = builder.load(ptr_u32, SourceSpan::default()).unwrap();

            // Check byte 0 (should be value1)
            let byte0_mask = builder.u32(0xff, SourceSpan::default());
            let byte0_after =
                builder.band(word_after_first, byte0_mask, SourceSpan::default()).unwrap();
            let expected_byte0 = builder.u32(0xaa, SourceSpan::default());
            builder.assert_eq(byte0_after, expected_byte0, SourceSpan::default()).unwrap();

            // Check byte 1 (should be 0xEE)
            let shift8 = builder.u32(8, SourceSpan::default());
            let byte1_shifted =
                builder.shr(word_after_first, shift8, SourceSpan::default()).unwrap();
            let byte1_after =
                builder.band(byte1_shifted, byte0_mask, SourceSpan::default()).unwrap();
            let expected_byte1 = builder.u32(0xee, SourceSpan::default());
            builder.assert_eq(byte1_after, expected_byte1, SourceSpan::default()).unwrap();

            // Check byte 2 (should still be 0xDD)
            let shift16 = builder.u32(16, SourceSpan::default());
            let byte2_shifted =
                builder.shr(word_after_first, shift16, SourceSpan::default()).unwrap();
            let byte2_after =
                builder.band(byte2_shifted, byte0_mask, SourceSpan::default()).unwrap();
            let expected_byte2 = builder.u32(0xdd, SourceSpan::default());
            builder.assert_eq(byte2_after, expected_byte2, SourceSpan::default()).unwrap();

            // Check byte 3 (should be 0xCC)
            let shift24 = builder.u32(24, SourceSpan::default());
            let byte3_shifted =
                builder.shr(word_after_first, shift24, SourceSpan::default()).unwrap();
            let byte3_after =
                builder.band(byte3_shifted, byte0_mask, SourceSpan::default()).unwrap();
            let expected_byte3 = builder.u32(0xcc, SourceSpan::default());
            builder.assert_eq(byte3_after, expected_byte3, SourceSpan::default()).unwrap();

            // Now store second u8 at offset 2
            let addr_plus_2 = builder.u32(write_to + 2, SourceSpan::default());
            let ptr_u8_offset = builder
                .inttoptr(
                    addr_plus_2,
            ...
            // After second store, check all bytes again
            let word_after_second = builder.load(ptr_u32, SourceSpan::default()).unwrap();

            // Check byte 0 (should still be value1)
            let byte0_final =
                builder.band(word_after_second, byte0_mask, SourceSpan::default()).unwrap();
            builder.assert_eq(byte0_final, expected_byte0, SourceSpan::default()).unwrap();

            // Check byte 1 (should still be 0xEE)
            let byte1_shifted_final =
                builder.shr(word_after_second, shift8, SourceSpan::default()).unwrap();
            let byte1_final =
                builder.band(byte1_shifted_final, byte0_mask, SourceSpan::default()).unwrap();
            builder.assert_eq(byte1_final, expected_byte1, SourceSpan::default()).unwrap();

            // Check byte 2 (should now be value2)
            let byte2_shifted_final =
                builder.shr(word_after_second, shift16, SourceSpan::default()).unwrap();
            let byte2_final =
                builder.band(byte2_shifted_final, byte0_mask, SourceSpan::default()).unwrap();
            let expected_byte2_final = builder.u32(0xbb, SourceSpan::default());
            builder
                .assert_eq(byte2_final, expected_byte2_final, SourceSpan::default())
                .unwrap();

            // Check byte 3 (should still be 0xCC)
            let byte3_shifted_final =
                builder.shr(word_after_second, shift24, SourceSpan::default()).unwrap();
            let byte3_final =
                builder.band(byte3_shifted_final, byte0_mask, SourceSpan::default()).unwrap();
            builder.assert_eq(byte3_final, expected_byte3, SourceSpan::default()).unwrap();
        ...
        let output = eval_link_output::<u32, _, _>(
            ...
            |trace| {
                ...
                // 1. After first store, only byte 0 changed
                // 2. After second store, only byte 2 changed (byte 0 remained as value1)
                ...
                println!("  After first store (offset 0): only byte 0 changed to 0x{:02x}", value1);
                println!(
                    "  After second store (offset 2): only byte 2 changed to 0x{:02x}",
                    value2
                );
                println!("  All other bytes remained unchanged throughout");
                println!("\nFinal memory state:");
...
fn store_u16_precise() {
    ...
    // Use specific values that are easily distinguishable
    // let value1: u16 = 0x1234;  // Store at offset 0
    // let value2: u16 = 0x5678;  // Store at offset 2
    let store_value1: u16 = 122; // Store at offset 0
    let store_value2: u16 = 9; // Store at offset 2
    ...
    let test_fn = || {
        ...
        // let write_to = 1048564;
        let write_to = 1048564;

        // TODO: WHY WTF it is green?

        let write_to_aligned = write_to - 4;

        // Initialize memory with a pattern that's different from what we'll write
        // This helps us detect any unintended modifications
        // Pattern: [0xFF, 0xEE, 0xDD, 0xCC, 0x11, 0x22, 0x33, 0x44]
        let initial_bytes =
            [0x00, 0x00, 0x00, 0x00, 0xff, 0xee, 0xdd, 0xcc, 0x11, 0x22, 0x33, 0x44];
        let initializers = [Initializer::MemoryBytes {
            addr: write_to_aligned,
            bytes: &initial_bytes,
        }];
        ...
        setup::build_entrypoint(link_output.component, &signature, |builder| {
            ...
            // Also check u16 at offset 4 is unchanged (0x2211)
            // let addr_plus_4 = builder.u32(write_to + 4, SourceSpan::default());
            // let ptr_u16_offset4 = builder
            // .inttoptr(addr_plus_4, Type::from(PointerType::new(Type::U16)), SourceSpan::default())
            // .unwrap();
            // let loaded_at_4 = builder.load(ptr_u16_offset4, SourceSpan::default()).unwrap();
            // let expected_at_4 = builder.u16(0x2211, SourceSpan::default());
            // builder.assert_eq(loaded_at_4, expected_at_4, SourceSpan::default()).unwrap();
            ...
            // // Check that u16 at offset 4 is still unchanged
            // let loaded_at_4_final = builder.load(ptr_u16_offset4, SourceSpan::default()).unwrap();
            // builder.assert_eq(loaded_at_4_final, expected_at_4, SourceSpan::default()).unwrap();
    ...
    let test_fn = || {
        ...
        let output = eval_link_output::<u32, _, _>(
            ...
            |_trace| {
                ...
                println!(
                    "Test passed: stored u16 value 0x{:04x} was successfully loaded back",
                    value
                );
                Ok(())
            },
