...
/// Builds the sibling+FPI scenario: a local counter (used as a sibling), a remote counter
/// exporting a distinct interface (reached through FPI), a caller depending on both, and the note.
///
/// The two counter components export distinct WIT interfaces (`counter-contract` and
/// `remote-counter`), so the caller's generated sibling trait and its `#[account]` FPI trait get
/// distinct Rust names — the trait-based `#[account]` macro cannot put two traits of the same name
/// (and the same interface imported with and without the `fpi-*` functions) in one crate.
pub(super) fn build_sibling_and_fpi_test_packages(
    test_name: &str,
    counter_source: &str,
    remote_counter_source: &str,
    caller_source: &str,
    note_source: &str,
) -> (
    Arc<Package>,
    ...
    let names = SiblingProjectNames::new(test_name);
    let remote_account_name = format!("{}-remote-counter-account", names.base_name);
    let remote_account_package = format!("miden:{remote_account_name}");

    let local_storage_slot = counter_storage_slot_name_for_package(&names.counter_account_package);
    let remote_storage_slot =
        storage_slot_name_for_package(&remote_account_package, "remote_counter");
    ...
    let remote_project = build_sibling_component_project(
        &remote_account_name,
        &remote_account_package,
        "remote-counter",
        remote_counter_source,
    );
    let remote_package = compile_rust_package(remote_project.root(), true);

    let counter_root = counter_project.root();
    let remote_root = remote_project.root();
    let dependencies = [
        (names.counter_account_package.as_str(), counter_root.as_path()),
        (remote_account_package.as_str(), remote_root.as_path()),
    ];
    let (caller_project, caller_package) =
        build_sibling_caller_project(&names, &dependencies, caller_source);
    let note_package =
