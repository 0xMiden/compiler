...
fn swapp_note_package_size() {
    ...
    expect!["38878"].assert_eq(stripped_mast_size_str(packages.swapp.as_ref()).as_str());
}
...
fn swapp_note_full_fill_transfers_assets() {
    ...
    expect!["12585"].assert_eq(single_note_cycles(executed_tx.measurements()));
...
fn swapp_note_partial_fill_creates_remainder_and_chains() {
    ...
    expect!["17655"].assert_eq(single_note_cycles(executed_tx.measurements()));
...
fn swapp_note_creator_reclaims_offered_asset() {
    ...
    expect!["5346"].assert_eq(single_note_cycles(executed_tx.measurements()));
