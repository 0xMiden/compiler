    ...
    rt.block_on(async {
        ...
        let p2id_note_mint = create_note_from_package(
            ...
            NoteCreationConfig {
                ...
                inputs: alice_account.id().to_felt_repr().into_iter().map(Into::into).collect(),
                ..Default::default()
            },
        );
    ...
    rt.block_on(async {
        ...
        let p2ide_note = create_note_from_package(
            ...
            NoteCreationConfig {
                ...
                inputs: {
                    let mut inputs: Vec<Felt> =
                        bob_account.id().to_felt_repr().into_iter().map(Into::into).collect();
