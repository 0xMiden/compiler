...
pub enum FrontendMetadata {
    ...
    AuthScript {
        ...
        method_path: String,
        /// Full Miden path of the export, as carried by its WIT `@external-id`.
        path: String,
    },
...
impl FrontendMetadata {
    ...
    /// Returns the full Miden path of the export marked by this metadata entry.
    pub fn path(&self) -> &str {
        match self {
            Self::AuthScript { path, .. }
            | Self::AccountProcedure { path, .. }
            | Self::NoteScript { path, .. }
            | Self::TxScript { path, .. } => path,
        }
    }
}
...
"#;
    ...
    fn frontend_metadata_roundtrips_payload() {
        let entries = [
            ...
            FrontendMetadata::NoteScript {
                ...
                path: "miden::payment::payment::note_script".to_string(),
            },
        ];
    ...
    fn frontend_metadata_matches_protocol_export_kind() {
        ...
        assert_eq!(
            note_metadata.protocol_export_kind_for("miden::payment::payment::note_script"),
            Some(ProtocolExportKind::NoteScript)
        );
        assert_eq!(note_metadata.protocol_export_kind_for("miden::auth::auth::other"), None);
    }
}
