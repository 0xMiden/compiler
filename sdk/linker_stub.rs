...
trait StubRet {
    /// Builds the return value from the current opaque argument sinks.
...
fn write_int_sink(value: u64) {
    unsafe {
        ...
        core::ptr::write_volatile(core::ptr::addr_of_mut!(STUB_FLOAT_ARG_SINK.int), value as u32);
    }
}
...
fn write_float_sink(value: f32) {
    unsafe {
        core::ptr::write_volatile(
            core::ptr::addr_of_mut!(STUB_INT_ARG_SINK),
            value.to_bits().into(),
        );
    }
}
...
trait StubArg {
    /// Writes this argument into the shared opaque sink state.
    fn write_sink(self);
}
...
/// Defines a public opaque linker stub with argument and return-value dependencies.
