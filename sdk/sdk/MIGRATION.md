...
read dependency WIT from the dependency's compiled `.masp`, so WIT path metadata is no longer read
and every Miden path dependency must be a built package (cargo-miden builds path dependencies
automatically; a dependency that points directly at a prebuilt `.masp` file is self-contained).
