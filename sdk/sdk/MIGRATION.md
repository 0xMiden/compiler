...
not a WIT keyword such as `list` or `type`. The component-model id used before is rejected by the
SDK macros with an error that shows the expected shape. `cargo miden new` writes
`miden::<package>::<package>` (package name snake-cased) for account, note and
transaction-script projects. The first two segments form the WIT package id, so the `pkg` segment
identifies the crate and must not be shared by two crates a consumer links.
...
changes the summary preimage from the six-word layout of protocol 0.16 to a versioned one: the
two parameter words move to the front, the expiration-delta felt gives way to `version` and
`metadata`, and the reference block number joins the preimage. That shifts every user parameter
down one slot and leaves six of them instead of seven. The host rebuilds the summary from the
advice-map preimage and rejects the signing request unless its commitment matches, so a component
that still hashes the old layout fails at runtime with `TransactionSummaryConstructionFailed`
even though it compiles unchanged. A component still on the four-word layout of protocol 0.15
can adopt the *after* code below directly.
...
Protocol 0.17 changes this layout again; see
...
above when moving on to `0.15.0`.
...
above for the required six-word layout.
...
- `faucet::{create_fungible_asset, create_non_fungible_asset, has_callbacks}` and the asset
  constructors `asset::{create_fungible_asset, create_non_fungible_asset}` are gone. The kernel
