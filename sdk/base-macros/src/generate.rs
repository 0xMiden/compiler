...
const FPI_FUNCTION_PREFIX: &str = "fpi_";
/// WIT function prefix used for foreign procedure imports.
const FPI_WIT_FUNCTION_PREFIX: &str = "fpi-";
    ...
    let functions = interface
        ...
        .filter(|function| {
            matches!(function.kind, FunctionKind::Freestanding)
                && !function.name.starts_with(FPI_WIT_FUNCTION_PREFIX)
        })
        .cloned()
        .collect::<Vec<_>>();

    for function in functions {
        let fpi_name = format!("{FPI_WIT_FUNCTION_PREFIX}{}", function.name);
...
fn fpi_method_ident(func: &ItemFn) -> syn::Result<syn::Ident> {
    ...
    let Some(method_name) = fn_name.strip_prefix(FPI_FUNCTION_PREFIX) else {
        return Err(Error::new(
            func.sig.ident.span(),
            format!("expected generated FPI function name to start with `{FPI_FUNCTION_PREFIX}`"),
        ));
    };
...
fn is_fpi_function(func: &ItemFn) -> bool {
    matches!(func.vis, syn::Visibility::Public(_))
        && func.sig.unsafety.is_none()
        && func.sig.ident.to_string().starts_with(FPI_FUNCTION_PREFIX)
}
