...
fn expand_note_impl(item_impl: ItemImpl) -> TokenStream2 {
    ...
    let constructor_guest_methods: Vec<TokenStream2> = constructors
        ...
        .collect();
    let entrypoint_root_impl = render_entrypoint_root_method(&note_ty, &entrypoint_fn);

    quote! {
        #runtime_boilerplate
        #item_impl

        #entrypoint_root_impl
...
/// Renders the generated `get_entrypoint_root()` associated method on the note input type.
///
/// Emitted from the `#[note]` impl expansion — not the struct expansion — so the method exists
/// exactly when a `#[note_script]` entrypoint exists. The method obtains the note script root
/// through a Rust function reference to the entrypoint method: the compiler resolves the
/// reference to its function-table slot, repoints the slot at the compiled note-script export,
/// and materializes the export's MAST root — the note script root the transaction kernel
/// observes — via a `procref` computed at assembly time.
fn render_entrypoint_root_method(
    note_ty: &syn::TypePath,
    entrypoint_fn: &ImplItemFn,
) -> TokenStream2 {
    let entrypoint_ident = &entrypoint_fn.sig.ident;
    // The fn-pointer type is written with one inferred parameter per entrypoint argument
    // (including the `self` receiver, which coerces to the note input type by value).
    let param_wildcards = entrypoint_fn
        .sig
        .inputs
        .iter()
        .map(|_| quote!(_))
    ...
    quote! {
        impl #note_ty {
            /// Returns the MAST root digest of this note's script — the note script root the
            /// transaction kernel observes when the note is executed.
            ///
            /// Generated by `#[note]`. The root is obtained through a function reference to the
            /// `#[note_script]` entrypoint method, which the compiler resolves to the compiled
            /// note-script export; the digest itself is computed at assembly time.
            #[inline(always)]
            pub fn get_entrypoint_root() -> ::miden::Word {
                // The function reference determines which procedure's MAST root is returned;
                // the compiler requires it to be a compile-time constant.
                let entrypoint: fn(#(#param_wildcards),*) = #note_ty::#entrypoint_ident;
                ::miden::note::__entrypoint_root_from_fn_ref(entrypoint as usize)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn entrypoint_root_method_takes_a_reference_to_the_entrypoint() {
        let note_ty: syn::TypePath = parse_quote!(MyNote);
        let entrypoint: ImplItemFn = parse_quote! {
            pub fn execute(self, _arg: Word, account: &mut Wallet) {}
        };

        let tokens = render_entrypoint_root_method(&note_ty, &entrypoint)
            .to_string()
            .replace(' ', "");

        assert!(tokens.contains("pubfnget_entrypoint_root()"), "unexpected expansion: {tokens}");
        // The fn-pointer coercion binds one inferred parameter per entrypoint argument,
        // including the receiver
        assert!(tokens.contains("fn(_,_,_)=MyNote::execute"), "unexpected expansion: {tokens}");
        assert!(
            tokens.contains("__entrypoint_root_from_fn_ref(entrypointasusize)"),
