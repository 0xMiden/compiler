...
struct Dependency {
    ...
    module_path: String,
    /// Generated trait name for this component, as written in the `package::Interface` reference.
...
///
/// Each selected component (`package::Interface` reference) expands to a `pub trait <Interface>`
/// whose default methods dispatch between the native (active) account and a foreign account, plus
/// an empty `impl <Interface> for <Wrapper>` that attaches it. Emitting one trait per component —
/// rather than inherent methods on the wrapper — lets two components that export the same method
/// name coexist on one wrapper, disambiguated by the caller with
/// `<Wrapper as Interface>::method(account, ..)`.
///
/// `trait_idents` runs parallel to `dependencies`: each entry is the trait name written in the
/// corresponding `package::Interface` reference.
pub(crate) fn augment_foreign_account_bindings(
    ...
    let dependencies = dependencies
        .into_iter()
        .zip(trait_idents)
        .map(|(dependency, trait_ident)| load_dependency(dependency, trait_ident))
        .collect::<syn::Result<Vec<_>>>()?;
    let struct_item = foreign_account_struct(&account_struct)?;
    let constructor_item = foreign_account_impl(&account_struct);
...
fn build_wrapper_method(
    ...
    // wrapper's private field, so the default method body needs no access to the struct internals
    // and a component method named `foreign_account_id` cannot shadow the dispatch.
    let dispatch = quote! {
