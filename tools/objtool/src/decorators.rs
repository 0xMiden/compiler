...
impl ArtifactKind {
    fn from_target_type(ty: TargetType) -> Self {
        match ty {
            TargetType::Executable => ArtifactKind::Program,
            _ => ArtifactKind::Library,
        }
    }
}
...
pub fn run(command: DecoratorsCommand) -> Result<()> {
    ...
    let report = Report {
        ...
        artifact_kind: ArtifactKind::from_target_type(package.kind),
        metric_points: vec![
