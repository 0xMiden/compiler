...
use super::core::registry::DependencyResolution;
...
impl UseTrie {
    ...
    fn insert_interface_type(
        ...
        self.insert(
            ["bindings", "exports", pkg.name.namespace.as_str(), pkg.name.name.as_str(), name],
            ty,
        )
    }
...
impl<'a> UnimplementedFunction<'a> {
    ...
    fn print(&self, trie: &mut UseTrie, source: &mut String) -> Result<()> {
        let (name, self_param, constructor) = match self.func.kind {
            ...
            FunctionKind::Method(_) => (
                to_rust_ident(self.func.name.split_once('.').expect("invalid method name").1)
                    .into(),
                true,
                false,
            ),
            ...
            | FunctionKind::AsyncStatic { .. } => {
                bail!("component-model-async unsupported: function of kind {:?}", self.func.kind)
            }
        };
            ...
            TypeDefKind::Type(Type::Id(kind_id)) => {
                ...
                match kind_type.owner {
                    ...
                    TypeOwner::Interface(interface_id) => {
                        self.target_world.exports.values().any(|world_item| match world_item {
    ...
    fn print_type_path(
        ...
        write!(source, "{name}", name = trie.insert(["bindings"], &type_name)).unwrap();
    }
    ...
    fn new(
        ...
        for (_, func) in interface.functions.iter() {
            let id = match func.kind {
                ...
                | FunctionKind::AsyncMethod { .. }
                | FunctionKind::AsyncStatic { .. } => {
                    bail!("component-model-async unsupported: function of kind {:?}", func.kind)
                }
            };
        ...
        for (key, item) in &world.exports {
            match item {
                ...
                WorldItem::Interface {
                    ...
                    interfaces
                        .push(InterfaceGenerator::new(resolve, key, interface, names, world)?);
                }
...
impl<'a> SourceGenerator<'a> {
    ...
    pub async fn generate(&self, world: Option<&str>) -> Result<String> {
        ...
        writeln!(&mut source)?;
        write!(&mut source, "{trie}{nl}", nl = if trie.is_empty() { "" } else { "\n" })?;
    ...
    async fn decode(&self, world: Option<&str>) -> Result<(Resolve, WorldId)> {
        ...
        let world = resolve.select_world(pkg_id, world).with_context(|| {
            format!("failed to select world from target package `{name}`", name = self.name)
        })?;
        Ok((resolve, world))
    }
}
