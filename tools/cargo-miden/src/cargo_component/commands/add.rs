...
impl AddCommand {
    ...
    pub async fn exec(self) -> Result<()> {
        ...
        let PackageComponentMetadata { package, metadata }: PackageComponentMetadata<'_> =
            match &spec {
                ...
                None => PackageComponentMetadata::new(
                    metadata.root_package().context("no root package found in metadata")?,
                )?,
            };
        ...
        if let Some(path) = self.path.as_ref() {
            ...
            config.terminal().status(
                "Added",
                format!("dependency `{name}` from path `{path}`", path = path.to_str().unwrap()),
            )?;
        } else {
            ...
            config
                .terminal()
                .status("Added", format!("dependency `{name}` with version `{version}`"))?;
        }
    ...
    async fn resolve_version(
        ...
        let dependency = Dependency::Package(RegistryPackage {
            ...
            version: self.package.version.as_ref().unwrap_or(&VersionReq::STAR).clone(),
        ...
        F: FnOnce(&mut Table) -> Result<()>,
    {
        let manifest = fs::read_to_string(&pkg.manifest_path).with_context(|| {
            format!("failed to read manifest file `{path}`", path = pkg.manifest_path)
        })?;

        let mut document: DocumentMut = manifest.parse().with_context(|| {
            format!("failed to parse manifest file `{path}`", path = pkg.manifest_path)
        })?;
        ...
        if self.dry_run {
            ...
            fs::write(&pkg.manifest_path, document.to_string()).with_context(|| {
                format!("failed to write manifest file `{path}`", path = pkg.manifest_path)
            })?;
        }
    ...
    fn add_from_path(&self, pkg: &Package, path: &Path) -> Result<()> {
        self.with_dependencies(pkg, |dependencies| {
            ...
            dependencies[&key] =
                value(InlineTable::from_iter([("path", Value::from(path.to_str().unwrap()))]));
            ...
            match &metadata.section.target {
                ...
                Target::Local { dependencies, .. } => {
                    if dependencies.contains_key(name) {
                        bail!(
                            "cannot add dependency `{name}` as it conflicts with an existing \
                             dependency"
                        );
                    }
                }
