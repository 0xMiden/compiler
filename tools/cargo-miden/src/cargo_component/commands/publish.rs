...
impl PublishCommand {
    ...
    pub async fn exec(self) -> Result<()> {
        ...
        let packages = [PackageComponentMetadata::new(if let Some(spec) = &spec {
            ...
            metadata.root_package().context("no root package found in manifest")?
        })?];
        ...
        let name = component_metadata.section.package.as_ref().with_context(|| {
            format!(
                "package `{name}` is missing a `package.metadata.component.package` setting in \
                 manifest `{path}`",
                name = package.name,
        ...
        if let Ok(key) = std::env::var("CARGO_COMPONENT_PUBLISH_KEY") {
            let registry = config
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "Tried to set a signing key, but registry was not set and no default \
                         registry was found. Try setting the `--registry` option."
                    )
                })?
                .to_owned();
            ...
            let reg_config = config.pkg_config.get_or_insert_registry_config_mut(&registry);
            let mut warg_conf = WargRegistryConfig::try_from(&*reg_config).unwrap_or_default();
            warg_conf.signing_key =
                Some(Arc::new(key.try_into().context("Failed to parse signing key")?));
        ...
        .await?;
        if outputs.len() != 1 {
            bail!("expected one output from `cargo build`, got {len}", len = outputs.len());
        }
