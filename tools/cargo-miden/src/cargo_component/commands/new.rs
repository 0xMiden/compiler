...
impl NewCommand {
    ...
    fn update_manifest(
        ...
        let manifest = fs::read_to_string(&manifest_path).with_context(|| {
            format!("failed to read manifest file `{path}`", path = manifest_path.display())
        })?;

        let mut doc: DocumentMut = manifest.parse().with_context(|| {
            format!("failed to parse manifest file `{path}`", path = manifest_path.display())
        })?;
        ...
        component["package"] = value(format!("{ns}:{name}", ns = name.namespace, name = name.name));
        ...
        fs::write(&manifest_path, doc.to_string()).with_context(|| {
            format!("failed to write manifest file `{path}`", path = manifest_path.display())
        })?;
        ...
        let status = cargo_add_command.status().context("failed to execute `cargo add` command")?;
        if !status.success() {
            bail!(
                "`cargo add {WIT_BINDGEN_RT_CRATE} --features bitflags` command exited with \
                 non-zero status"
            );
        }
        ...
        config
            .terminal()
            .status("Updated", format!("manifest of package `{name}`", name = name.display))?;
...
"#
    ...
    fn create_source_file(
        ...
        fs::write(&source_path, source).with_context(|| {
            format!("failed to write source file `{path}`", path = source_path.display())
        })?;
        ...
        match target {
            ...
            None => {
                config.terminal().status("Generated", format!("source file `{path}`"))?;
            }
        }
...
"#,
        ...
        .with_context(|| format!("failed to write targets file `{path}`", path = path.display()))
    }
    ...
    async fn resolve_target(
        ...
        match target {
            Some(metadata::Target::Package {
                ...
                Ok(Some((
                    dependencies.into_values().next().expect("expected a target resolution"),
                    world,
                )))
            }
            _ => Ok(None),
