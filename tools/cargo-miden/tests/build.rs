...
fn build_new_project_from_template(template: &str) -> Package {
    ...
    let new_project_path = match output {
        cargo_miden::CommandOutput::NewCommandOutput { project_path } => {
            ...
            let abs_path = project_path.absolutize()
                .map(|p| p.to_path_buf())
                .unwrap_or_else(|_| expected_new_project_dir.clone());
            // Use expected path if the returned path doesn't exist or can't be canonicalized
            if abs_path.exists() {
                abs_path.canonicalize().unwrap_or(expected_new_project_dir.canonicalize().unwrap())
            } else {
...
fn new_project_integration_tests_pass() {
    ...
    let project_path = match output {
        cargo_miden::CommandOutput::NewCommandOutput { project_path } => {
            // Convert to absolute path if relative
            use path_absolutize::Absolutize;
            let expected_path = temp_dir.join(&project_name);
            let abs_path = project_path.absolutize()
                .map(|p| p.to_path_buf())
                .unwrap_or_else(|_| expected_path.clone());
            // Use expected path if the returned path doesn't exist or can't be canonicalized
            if abs_path.exists() {
                abs_path.canonicalize().unwrap_or(expected_path.canonicalize().unwrap())
            } else {
