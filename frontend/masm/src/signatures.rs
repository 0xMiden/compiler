...
pub(crate) fn convert_signature_with_external_types(
    ...
    let params = convert_stack_types(context, module, &signature.args, external_types)?;
    let results = convert_stack_types(context, module, &signature.results, external_types)?;
...
pub(crate) fn convert_hir_function_type(
    ...
    for ty in signature.params() {
        push_hir_signature_stack_types(ty.clone(), &mut params);
    }
    let mut results = Vec::new();
    for ty in signature.results() {
        push_hir_signature_stack_types(ty.clone(), &mut results);
    }

    Signature::with_convention(context, signature.calling_convention(), params, results)
}
...
fn convert_stack_types(
    ...
    for ty_expr in types {
        let ty = convert_type_expr_with_external_types(context, module, ty_expr, external_types)?;
        if preserves_stack_value(context, module, ty_expr, external_types)? {
            stack_types.push(ty);
        } else {
            push_stack_types(ty, &mut stack_types);
        }
    }
    Ok(stack_types)
}

fn push_stack_types(ty: Type, stack_types: &mut Vec<Type>) {
    match ty {
        Type::I64 | Type::U64 => {
            stack_types.extend(std::iter::repeat_n(Type::U32, 2));
        }
        Type::I128 | Type::U128 => {
            stack_types.extend(std::iter::repeat_n(Type::U32, 4));
        }
        Type::U256 => {
            stack_types.extend(std::iter::repeat_n(Type::U32, 8));
        }
        Type::Array(array) => {
            for _ in 0..array.len() {
                push_stack_types(array.element_type().clone(), stack_types);
            }
        }
        Type::Struct(struct_ty) => {
            for field in struct_ty.fields() {
                push_stack_types(field.ty.clone(), stack_types);
            }
        }
        ty => stack_types.push(ty),
    }
}

fn push_hir_signature_stack_types(ty: Type, stack_types: &mut Vec<Type>) {
    if matches!(ty, Type::Array(_)) {
        stack_types.push(ty);
    } else {
        push_stack_types(ty, stack_types);
    }
}

fn preserves_stack_value(
    context: &Context,
    module: &Module,
    ty: &TypeExpr,
    external_types: &ExternalTypeMap,
) -> Result<bool> {
    preserves_stack_value_with_depth(context, module, ty, external_types, 0)
}

fn preserves_stack_value_with_depth(
    context: &Context,
    module: &Module,
    ty: &TypeExpr,
    external_types: &ExternalTypeMap,
    ...
    match ty {
        TypeExpr::Array(_) => Ok(true),
        TypeExpr::Ref(path) => {
            preserves_stack_ref_value(context, module, path.clone(), external_types, depth + 1)
        }
        TypeExpr::Primitive(_) | TypeExpr::Ptr(_) | TypeExpr::Struct(_) => Ok(false),
    }
}

fn preserves_stack_ref_value(
    context: &Context,
    module: &Module,
    ...
    loop {
        ...
        match module.resolve_path(path.as_deref(), source_manager.clone()) {
            Ok(SymbolResolution::Local(item)) => {
                ...
                return preserves_stack_value_with_depth(
                    context,
                    module,
                    &decl.ty(),
                    external_types,
            ...
            Ok(SymbolResolution::External(resolved)) => {
                ...
                if let Some(ty) = external_types.get(resolved_key) {
                    return Ok(matches!(ty, Type::Array(_)));
                }
                if resolved != path {
                    path = resolved;
                    continue;
