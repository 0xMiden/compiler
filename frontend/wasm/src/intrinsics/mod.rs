...
pub fn convert_intrinsics_call<B: ?Sized + Builder>(
    ...
    match intrinsic {
        ...
        // Note intrinsics require module context (function tables and frontend metadata), so
        // call sites dispatch them to `note::convert_note_intrinsics_call` directly
        Intrinsic::Note(function) => {
            panic!(
                "note intrinsic '{function}' must be lowered via \
                 `note::convert_note_intrinsics_call`"
            )
        }
    }
}
