...
impl Module {
    ...
    /// Ensures each function in the module has a unique name.
    ///
    /// WebAssembly function names in the [name section] are not guaranteed to be unique. This
    /// method rewrites duplicate names so that every function has a unique name.
    ///
    ...
    pub fn sanitize_duplicate_func_names(
        ...
        for (index, name) in self.name_section.func_names.iter_mut() {
