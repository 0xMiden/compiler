...
- Imported tables, element segments with `global.get`-relative offsets, table mutation ops
  (`table.set`, `table.get`, `table.grow`, ...), and `ref.func`/`ref.null` as function body
  instructions; `call_indirect` through locally-defined `funcref` tables *is* supported (see the
  "Function call indirection" appendix of the compiler docs for details and limitations);
