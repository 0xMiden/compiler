...
impl RewritePattern for ConvertTrivialIfToSelect {
    ...
    fn match_and_rewrite(
        ...
        // Create a new `hir.if` for the non-hoistable results, if there are any.
        //
        // Then, use either the new `hir.if`, or the original, as the anchor for inserting hoisted
        // `hir.select`s.
        let anchor = if !non_hoistable.is_empty() {
            // Create a new `hir.if` with the non-hoistable results
            let ins = DefaultInstBuilder::new(rewriter);
            let mut new_if = ins.r#if(cond, &non_hoistable, span)?;
            ...
            // new one, we can simply insert all of the selects and then erase the hir.if
            if_op_ref
        };

        // Insert `hir.select` ops for each hoisted result
            ...
            then_yield_args.iter().copied().zip(else_yield_args.iter().copied())
        {
            ...
            } else if let Some(duplicate) = deduplicated_selections.get(&(true_value, false_value))
            {
                ...
                let ins = DefaultInstBuilder::new(rewriter);
                let selected = ins.select(cond, true_value, false_value, span)?;
                results.push(Some(selected));
                deduplicated_selections.insert((true_value, false_value), selected);
            }
        }

        // If we have non-hoistable values, rewrite the `hir.yield` ops in the new `hir.if`
        if !non_hoistable.is_empty() {
            ...
            let replacement_then_yield =
                DefaultInstBuilder::new(rewriter).r#yield(true_yields, span)?.as_operation_ref();
            ...
            let replacement_else_yield =
                DefaultInstBuilder::new(rewriter).r#yield(false_yields, span)?;
            rewriter.replace_op(
