...
impl OpEmitter<'_> {
    ...
    /// Push the MAST root digest of `callee` — the function occupying a statically-resolved
    /// function-table slot — onto the operand stack as one word.
    ///
    /// This emits a `procref` instruction; the assembler computes the digest at assembly time
    /// and pushes it with `root[0]` on top.
    pub fn function_table_root(&mut self, callee: masm::InvocationTarget, span: SourceSpan) {
        for _ in 0..4 {
            self.push(Type::Felt);
        }
