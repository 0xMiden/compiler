...
/// Creates a P2ID output note and moves the given asset into it.
///
...
/// recipient — including the note script root — is computed by the `p2id` note package's
/// exported constructor. The note itself is created from that recipient through the account's
...
fn run(arg: Word, account: &mut Wallet) {
    ...
    let asset = Asset::new(Word::from(asset_id), asset_value);
