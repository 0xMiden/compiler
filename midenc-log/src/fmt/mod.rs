...
//! use std::io::Write;
//!
//! let mut builder = midenc_log::Builder::new();
//!
...
use std::{cell::RefCell, fmt, fmt::Display, io, io::prelude::Write, mem, rc::Rc};
...
pub use crate::writer::{Target, WriteStyle};
...
/// An instance of a `Formatter` is passed to an `midenc_log` format as `buf`:
///
/// ```
/// use std::io::Write;
///
    ...
    pub fn default_level_style(&self, level: Level) -> style::Style {
        if self.write_style == WriteStyle::Never {
            ...
            match level {
                ...
                Level::Error => style::AnsiColor::Red.on_default().effects(style::Effects::BOLD),
            }
        }
    }
}
...
impl ConfigurableFormatWriter<'_> {
    ...
    fn write_source_location(&mut self, record: &Record<'_>) -> io::Result<()> {
        ...
        if let Some(file_path) = record.file() {
            let line = self.format.source_line_number.then(|| record.line()).flatten();
            match line {
    ...
    fn write_kv(&mut self, record: &Record<'_>) -> io::Result<()> {
        let format = self.format.kv_format.as_deref().unwrap_or(&default_kv_format);
...
mod tests {
    ...
    fn formatter() -> Formatter {
        let writer = crate::writer::Builder::new().write_style(WriteStyle::Never).build();
    ...
    fn format_kv_default_full() {
        ...
        assert_eq!("[INFO  test::path test.rs:42 target] log\nmessage a=1 b=2\n", written);
    }
}
